...
while True:
    driver.get(r'https://www.flipkart.com/sony-playstation-5-cfi-1008a01r-825-gb-astro-s-playroom/p/itma0201bdea62fa')
    content = driver.page_source
    soup = bs(content)
    #print(soup)
    availability = soup.find("div", {"class": "_16FRp0"})
    if not any('Coming Soon' in s for s in str(availability).split('\n')):
        ctypes.windll.user32.MessageBoxW(0, "ps5 available on flipkart", "PS5 available Flipkart", 1)
        break
    else: 
        print('not available')